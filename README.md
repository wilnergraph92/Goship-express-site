...
- **Mon solde et mes messages** (dans « Mon compte ») : en tête des factures, le total facturé, le déjà payé et le reste à payer, avec le montant en retard et la prochaine échéance ; dans la colonne de droite, les derniers e-mails, messages WhatsApp et notifications que Goship Express lui a envoyés. Les chiffres sont calculés par la base (`mon_resume`), les mêmes que ceux de chaque facture, et ne portent que sur son propre compte.
...
Le tableau de bord a un **menu à gauche** (Vue générale, Colis, Clients, Factures, Scanner, Équipe — selon le rôle ; réductible aux icônes sur ordinateur, tiroir ☰ sur téléphone et tablette) et une **barre en haut** : le nom de l'onglet, **la date et l'heure** (toujours affichées), la recherche rapide, « Enregistrer un colis », la **cloche des alertes**, les **réglages** et le compte. Les **Réglages** (roue dentée) rangent en quatre rubriques : *Affichage* (période de la vue générale, lignes par page, seuil « sans mouvement », menu réduit, secondes de l'horloge — gardés sur l'appareil), *Mon compte* (rôle et permissions), *Poste de scan* (lieu, mode, son) et *Système*. Aucun réglage ne change une donnée ni une permission.

- **Vue générale** (premier onglet) : l'état de l'activité sur une période — aujourd'hui, 7 ou 30 derniers jours, ce mois-ci, le mois précédent, cette année ou deux dates au choix, comptées en jours de Santo Domingo. Colis reçus, livrés, en cours, en « action requise » et sans mouvement ; clients inscrits et nouveaux ; facturé, encaissé, à encaisser et en retard (gérant et administrateur) ; trois graphiques (colis par jour, statuts, facturé et encaissé par jour) ; les alertes ; l'activité récente ; l'activité du poste de scan (par employé, par opération, par jour) ; les derniers paiements ; et la liste des **colis à traiter** (action requise, ou sans aucun événement depuis 3, 7, 14 ou 30 jours), chacun ouvrable d'un clic. Voir « Le tableau de bord ».
- **Recherche rapide** (sous le titre, sur tous les onglets) : un numéro de colis ou de facture, son début, le suivi du vendeur, le code-barres ou le QR d'une étiquette, le code GSE d'un client, un mot de son nom, les derniers chiffres de son téléphone, le début de son e-mail. Un colis trouvé montre son client, sa destination, son poids, son statut, son dernier événement, sa facture et son solde.
...
- Rechercher un colis ou un client ; filtrer les colis par statut, service, destination et dates de réception (bouton « Réinitialiser les filtres ») ; 10, 25, 50 ou 100 lignes par page ; voir les colis d'un client ; corriger ou supprimer un colis.
- **Clients** : chaque client avec ses colis en cours (par statut), leur poids, ce qui lui a été facturé, ce qu'il a payé et son solde — comptés par la base, pour tous les clients — triables (récents, solde, activité, colis en cours, nom) et filtrables (avec un solde, avec des colis en cours).
...
`outils/supabase-tableau-de-bord.sql` (Phase 7) ne crée aucune table et ne change aucune règle : il ajoute des **fonctions de lecture** et des **index**. Chaque chiffre de la vue générale est compté par la base, sur les vraies tables, avec leurs règles — le solde est celui de `solde_usd`, l'état celui de `etat_paiement`, un statut est l'un des huit, un scan est un événement du moteur. La page n'additionne rien. Avant, le résumé des clients était fait dans la page à partir des 500 derniers colis et des 1 000 dernières factures : il se trompait au-delà.

| Fonction | Ce qu'elle rend | Permission |
|---|---|---|
| `vue_generale(période, début, fin, jours)` | les chiffres, courbes, alertes, activité, scans et paiements de la période | `shipments.view` ; chaque partie la sienne : `clients.view`, `reports.view` (facturation), `shipments.view_history` (scanner, activité), `payments.view` (paiements) |
| `colis_a_traiter(liste, jours, limite, décalage)` | les colis en « action requise » ou sans mouvement, les plus anciens d'abord | `shipments.view` |
| `recherche_rapide(texte)` | colis, clients et factures correspondants | `shipments.view` ; `clients.view`, `invoices.view` pour leurs parties |
| `clients_soldes(recherche, tri, filtre, limite, décalage)` | une page de clients avec leurs colis et leurs montants | `clients.view` ; montants avec `invoices.view` |
| `mon_resume()` | pour le client connecté : ses colis, son solde, ses derniers messages | son propre compte |

**Définitions.** *Reçus* : colis dont la réception à Miami tombe dans la période. *Livrés* : colis passés à « Livré » pendant la période, une fois chacun, sans ceux dont la livraison a été annulée par une correction. *Sans mouvement* : pas encore livré, et aucun événement (scan, changement de statut, correction) depuis le nombre de jours choisi. *Facturé* : les factures émises pendant la période, hors annulées — *payé* et *reste* disent ce qui en a été réglé, et facturé = payé + reste, toujours. *Encaissé* : l'argent reçu pendant la période, quelle que soit la facture. *À encaisser* et les états (à payer, payée en partie, en retard, payée, annulée) : toutes les factures, maintenant. *Opérations au scanner* : les événements écrits depuis le poste de scan.

**Ce qui n'a pas de chiffre, parce que la base ne le connaît pas** : les dépenses, les dettes et le résultat ; les scans échoués (un code illisible ou introuvable n'écrit rien) ; l'employé qui a enregistré un colis, en filtre de la liste des colis (il est dans l'historique de chaque colis, pas sur le colis) ; l'origine (toujours l'entrepôt de Miami). Ces cases n'existent pas, plutôt que d'afficher un zéro trompeur.

**Alertes** : colis en « action requise » (critique) ; colis sans mouvement ; factures échues non soldées (gérant, administrateur) ; colis d'un client sur aucune facture active. Aucune règle déclenchée : « Aucune alerte critique ».

**Mise à jour** : le bouton « Actualiser » ; l'ouverture de l'onglet si les chiffres ont plus d'une minute ; et les changements en direct, au plus une fois toutes les quinze secondes. Pas d'interrogation répétée de la base.

**Recherche par index** : numéro de colis ou de facture (début compris), suivi du vendeur, code client, mots du nom (début de mot), fin du téléphone (4 chiffres au moins), début de l'e-mail. Un « % » ou une lettre seule ne liste jamais tout ; au plus 25 réponses par partie.

## Analytics

Le tableau de bord dit **ce qui se passe maintenant** ; l'onglet **Analytics** (`admin.html#analytics`) dit **ce qui s'est passé et comment cela évolue**, en comparant chaque période à la précédente. `outils/supabase-analytics.sql` (Phase 8) ne crée aucune table et ne change aucune règle : des **fonctions de lecture** et deux index. Tout est compté par la base, sur les vraies tables ; la page met en forme et n'additionne rien.

**Qui la voit** : la permission `reports.view` (administrateur, gérant), déjà celle des chiffres de facturation. Aucune permission nouvelle. Un employé n'a pas l'onglet, et la base le refuse même s'il appelle les fonctions directement ; un client et un visiteur aussi.

| Rubrique | Fonction | Ce qu'elle montre |
|---|---|---|
| Synthèse | `analytics_synthese` | les chiffres de la période et de la précédente, les tendances décrites en phrases, les colis par statut maintenant |
| Expéditions | `analytics_synthese` + `analytics_serie` | reçus, expédiés, livrés, poids, par jour, semaine ou mois |
| Opérations | `analytics_operations` | délais entre étapes (moyenne, médiane, minimum, maximum, valeurs extrêmes signalées), transitions, taux de livraison, colis sans progression (24 h, 48 h, 72 h, 7 jours), action requise |
| Clients | `analytics_clients` | inscrits, nouveaux, actifs, inactifs, segments à seuils fixes, clients les plus actifs (triables) |
| Finances | `analytics_finances` + `analytics_serie` | facturé, encaissé, reste, créances et leur âge, moyens de paiement |
| Routes | `analytics_routes` | pays × service, villes, taux de livraison et délai par route |
| Scanner | `analytics_scanner` | opérations par type, par compte (ordre alphabétique : ce n'est pas un classement), par lieu |
| Qualité des données | `analytics_qualite` | champs manquants, incohérences, anomalies de facturation — sur toute la base |

**Périodes** : aujourd'hui, 7 et 30 jours, ce mois, mois précédent, 3, 6 et 12 derniers mois, cette année, personnalisée (trois ans au plus). En jours de Santo Domingo, comme le tableau de bord. **Période précédente** : autant de jours juste avant ; pour « ce mois », le mois précédent aux mêmes dates ; pour « cette année », l'année précédente aux mêmes dates. Une variation contre zéro s'écrit « nouveau », jamais « Infinity % ».

**Définitions** (les mêmes que la vue générale) : *reçus* — réception à Miami dans la période ; *expédiés, disponibles, livrés, action requise* — colis passés à ce statut pendant la période, une fois chacun, sans les étapes annulées par une correction ; *facturé* — factures émises dans la période, hors annulées, au montant arrêté à la création (un changement de tarif ne modifie aucun chiffre passé) ; *encaissé* — paiements valides reçus dans la période ; *créances* — ce qui restait dû à une date, reconstitué à partir des factures et des paiements datés ; *client actif* — au moins un colis reçu dans la période ; *taux de livraison* — colis reçus dans la période et livrés depuis. Chaque rubrique a son encadré « Définitions et sources ».

**Ce qui n'a pas de chiffre, parce que la base ne le connaît pas** : dépenses, dettes et résultat (l'encaissé n'est pas un bénéfice) ; scans refusés ou illisibles ; origine (toujours l'entrepôt de Miami) ; cause structurée d'une « action requise » (la note est un texte libre : ses notes les plus fréquentes sont listées telles qu'écrites). Aucune prévision, aucune explication causale.

**Fraîcheur** : les chiffres sont calculés à l'ouverture d'une rubrique ; une réponse est gardée une minute pour passer d'une rubrique à l'autre, et « Actualiser » l'oublie. Pas de temps réel : l'heure du calcul est affichée.
